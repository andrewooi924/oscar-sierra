/**
 * task3.c : A Deadline-based Scheduling Program. The program utilises a queue and works on processes from the top of the queue
 * to the bottom of the queue. The program sorts the queue/list for every process added in ascending order of deadlines to
 * prioritise processes with shorter deadlines. The program optionally accepts a process data file as input.
 * Name: Ooi Yu Zhang
 * Student ID: 32713339
...
void insertionSort(pcb_t processes[], int counter) {
  pcb_t temp; //Temporary container for process deadline during sorting
  int j;
  ...
  for (int i = 1; i < counter; i++) {
    temp = processes[i];
    j = i - 1;
    while (j >= 0 && (processes[j].deadline > temp.deadline)) {
  ...
  //Check if top process is still the previous running processes
  if (processes[0].state == READY) {
    for (int i = 0; i < counter; i++) {
  ...
  while (time <= sumTime) {
    ...
    while (fscanf(fp, "%s %d %d %d\n", processName, &arrivalTime, &serviceTime, &deadline) != EOF) {
      ...
      if (time == arrivalTime) {
        ...
        //Sort process list in ascending order of deadline
        insertionSort(processList, counter);
      }
    }
