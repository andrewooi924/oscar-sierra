/**
 * task2.c : A Round Robin Scheduling Program with a time quantum of 2. The program utilises a queue to schedule processes and
 * and works on processes from the top of the queue to the bottom of the queue. The program switches processes every two
 * seconds and optionally accepts a process data file as input.
 * Name: Ooi Yu Zhang
 * Student ID: 32713339
 * Start Date: 30/9/2022
 ...
 * Returns the new time quantum after processing the running process
 */
int processRunning(pcb_t processes[], int counter, int quantum) {
  ...
  if (processes[0].state == RUNNING) {
    ...
    //Check if 2 seconds has passed (time quantum 2)
    if (quantum == 2 && processes[0].remainingTime != 0) {
      //Switch to READY state
      processes[0].state = READY;
      //Move current process to end of queue
      processes[counter] = processes[0];
      //Push all processes
...
int main(int argc, char *argv[]) {
  ...
  //Start pulling processes from the file line by line
  while (time <= sumTime) {
    ...
    quantum = processRunning(processList, counter, quantum);
